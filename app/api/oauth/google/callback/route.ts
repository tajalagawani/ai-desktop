...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!provider) throw new Error('Google OAuth not configured. Please configure in Security Center first.')
    ...
    return new NextResponse(`<html><head><style>body{font-family:system-ui;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:linear-gradient(135deg,#667eea,#764ba2);color:white}.c{text-align:center;background:rgba(255,255,255,0.1);padding:40px;border-radius:20px;backdrop-filter:blur(10px)}.check{font-size:64px;animation:pop .3s}@keyframes pop{0%{transform:scale(0)}50%{transform:scale(1.2)}100%{transform:scale(1)}}</style></head><body><div class="c"><div class="check">✓</div><h1>Google Connected!</h1><p>Closing...</p></div><script>if(window.opener)window.opener.postMessage({type:'oauth_success',provider:'google'},'*');setTimeout(()=>window.close(),2000)</script></body></html>`, { headers: { 'Content-Type': 'text/html' } })
