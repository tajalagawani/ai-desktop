...
async function spawnClaude(command, options = {}, ws) {
  return new Promise(async (resolve, reject) => {
    const { sessionId, projectPath, cwd, resume, toolsSettings, permissionMode, images, onSessionCreated, onSessionComplete } = options;
    ...
    const claudeProcess = spawnFunction(claudePath, args, {
      ...
      env: {
        ...
        ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY, // Explicitly pass YOUR API key from .env
        ...(capturedSessionId && { CLAUDE_SESSION_ID: capturedSessionId }) // Pass session ID if available
      }
    });
    ...
    claudeProcess.stdout.on('data', (data) => {
      ...
      for (const line of lines) {
        try {
          ...
          // Filter out debug/technical messages - only send user-facing content
          const debugPrefixes = ['📤', '📄', '🔍', '✅', '❌', '⚠️', '📝', '🔒', '📡', '🆕', '🛑'];
          const startsWithDebugEmoji = debugPrefixes.some(prefix => line.startsWith(prefix));
          const isDebugMessage = startsWithDebugEmoji ||
                                 line.includes('Claude CLI stdout:') ||
                                 line.includes('Parsed JSON response:') ||
                                 line.includes('Non-JSON response:');

          // Only send non-debug, non-empty lines to the chat
          if (!isDebugMessage && line.trim()) {
            ws.send(JSON.stringify({
              type: 'claude-output',
              data: line
            }));
    ...
    claudeProcess.on('close', async (code) => {
      ...
      if (onSessionComplete && finalSessionId) {
        await onSessionComplete(finalSessionId);
      }
